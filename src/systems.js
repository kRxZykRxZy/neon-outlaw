...
import {WEAPONS,VEHICLES,MISSIONS,getMission} from './content.js';
const merge=(a,b)=>({...a,...b,settings:{...(a.settings||{}),...(b.settings||{})},upgrades:{...(a.upgrades||{}),...(b.upgrades||{})},stats:{...(a.stats||{}),...(b.stats||{})}});
...
export class EnemyManager{
 ...
 spawn(x,z,kind='gang'){const e=new Enemy(this.scene,this.world,this.player,x,z,kind);this.enemies.push(e);(this.scene.userData.enemyMeshes||(this.scene.userData.enemyMeshes=[])).push(e.mesh);return e}
...
export class MissionSystem{
 ...
 update(dt){if(!this.active)return;const p=this.player.inCar?this.player.car.group.position:this.player.group.position;const step=p.distanceTo(this.lastPos);this.distance+=step;this.save.data.stats.distance+=step;this.lastPos.copy(p);const m=this.active;let done=false;if(['drive','delivery','race'].includes(m.type))done=this.distance>=m.distance;if(['combat','boss'].includes(m.type))done=this.kills>=m.kills;if(m.type==='escape')done=this.distance>=m.distance&&this.player.save.data.wanted<=0;if(m.type==='endgame')done=this.save.data.completed.length>=6;if(done)this.complete()}
 onKill(){this.kills++}
